...
public class FirestoreEventsDataRepository implements IEventsDataRepository {
    ...
    * */
    ...
    public void updateEventInvitationKey(@NonNull String eventID, @NonNull String key, @NonNull Callback<String> callback) {
        firestore.collection(EVENT_COL).whereEqualTo(Event.EVENT_FIELD_INVITKEY, key).get().addOnCompleteListener(task1 -> {
    ...
    public void joinEvent(@NonNull String invitationKey, @NonNull Callback<String> callback) {
        ...
        firestore.collection(EVENT_COL).whereEqualTo(Event.EVENT_FIELD_INVITKEY, invitationKey).get().addOnCompleteListener(task1 -> {
            if (task1.isSuccessful() && task1.getResult() != null && task1.getResult().getDocuments().size() == 1) {
                ...
                writeBatch.set(eventDocRef, new HashMap<String, Object>(){{put(Event.EVENT_FIELD_INVITKEY, FieldValue.arrayUnion(userID));}}, SetOptions.merge());
    ...
    public void quitEvent(@NonNull String eventID, @NonNull Callback<Void> callback) {
        ...
        writeBatch.set(eventDoc, new HashMap<String, Object>(){{put(Event.EVENT_FIELD_INVITKEY, FieldValue.arrayRemove(user.getUid()));}}, SetOptions.merge());
