...
//        });
    ...
    public void deleteEventInvitationKey(@NonNull String eventID, @NonNull Callback<Void> callback) {
        firestore.collection(EVENT_COL).document(eventID)
                .set(new HashMap<String, Object>() {{
                    put(EVENT_FIELD_INVITKEY, FieldValue.delete());
                }}, SetOptions.merge())
        ...
        firestore.collection(EVENT_COL).whereEqualTo(EVENT_FIELD_INVITKEY, invitationKey).get().addOnCompleteListener(task1 -> {
            if (task1.isSuccessful() && task1.getResult() != null && task1.getResult().getDocuments().size() == 1) {
                ...
                writeBatch.set(userDocRef, new HashMap<String, Object>() {{
                    put(USERS_FIELD_EVENTS, FieldValue.arrayUnion(eventID));
                }}, SetOptions.merge());
                writeBatch.set(eventDocRef, new HashMap<String, Object>() {{
                    put(EVENT_FIELD_PARTICIPANTS, FieldValue.arrayUnion(userID));
                }}, SetOptions.merge());
    ...
    public void quitEvent(@NonNull String eventID, @NonNull Callback<Void> callback) {
        ...
        writeBatch.set(userDoc, new HashMap<String, Object>() {{
            put(USERS_FIELD_EVENTS, FieldValue.arrayRemove(eventID));
        }}, SetOptions.merge());
        writeBatch.set(eventDoc, new HashMap<String, Object>() {{
            put(EVENT_FIELD_PARTICIPANTS, FieldValue.arrayRemove(user.getUid()));
        }}, SetOptions.merge());
